...
func TestUploadSpanMapsToDomain(t *testing.T) {
	...
	spans, err := (UploadSpanData{Spans: []UploadSpan{{StartedATMicros: 1_000_000, DurationMicros: 250, TraceID: "trace", SpanID: "span", ParentID: "parent", SpanName: "model", SpanType: "model", StatusCode: 500, Input: "in", Output: "out", TagsString: map[string]string{"model": "x"}, TagsLong: map[string]int64{"tokens": 3}}}}).Map("workspace", received)
	if err != nil {
		t.Fatal(err)
	}
	if len(spans) != 1 {
	...
	span := spans[0]
	if span.WorkspaceID != "workspace" || span.TraceID != "trace" || span.ParentSpanID != "parent" || span.Name != "model" || span.Status != "error" {
