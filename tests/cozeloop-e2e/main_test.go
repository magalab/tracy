...
func TestOfficialGoSDKIngest(t *testing.T) {
	...
	if base == "" || token == "" || workspace == "" { t.Skip("set COZELOOP_API_BASE_URL, COZELOOP_API_TOKEN and COZELOOP_WORKSPACE_ID") }
	...
	span.SetInput(ctx, map[string]string{"hello": "tracy"}); span.SetOutput(ctx, map[string]string{"ok": "true"}); span.SetServiceName(ctx, "tracy-e2e"); span.Finish(ctx)
	client.Close(ctx)
}
