...
type DashboardMetrics = { request_count: number; error_count: number; error_rate: number; input_tokens: number; output_tokens: number; p95_latency_ms: number };
...
function App() {
  ...
  const [annotationKey, setAnnotationKey] = useState("quality");
  ...
  async function loadPage(nextCursor?: string) { if (!token) return; setLoading(true); setError(""); try { const suffix = nextCursor ? `&cursor=${encodeURIComponent(nextCursor)}` : ""; const next = await request<Page>(`/api/v1/traces?limit=50${suffix}`, token); setPage({ ...next, items: next.items ?? [] }); setCursor(next.next_cursor); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setLoading(false); } }
  ...
  useEffect(() => { if (token) { void loadPage(); void loadMetrics(); } }, [token]);
